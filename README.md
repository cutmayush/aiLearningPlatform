...
Run integration tests and a small API smoke test (recommended during development).
...
2. Install dependencies (includes test tools):
```powershell
pip install -r requirements.txt
```

3. Run unit/integration tests (pytest):
```powershell
pytest -q
```

4. Run a quick API smoke test that registers/logs-in and fetches recommendations:
```powershell
python test_api.py
```

Notes:
- The test suite uses a temporary SQLite file database so it won't overwrite `learning_agent.db`.
- `test_api.py` uses `requests` and a real running server; ensure the Flask app is running for that script.

---

## 📝 Changelog

See [CHANGELOG.md](CHANGELOG.md) for recent changes and fixes (tests added, recommendations improved).
