...
def get_recommendations():
    ...
    # Get user's current semester (fall back to 1)
    cursor.execute('SELECT current_semester FROM student_profiles WHERE user_id = ?', (user_id,))
    sem_row = cursor.fetchone()
    current_semester = sem_row['current_semester'] if sem_row else 1

    # Get weak areas (topics with low scores) limited to current semester
    cursor.execute('''
        SELECT t.*, s.name as subject_name, up.score, up.completion_status
        FROM user_progress up
        JOIN topics t ON up.topic_id = t.id
        JOIN subjects s ON t.subject_id = s.id
        WHERE up.user_id = ? AND up.score < 70 AND s.semester = ?
    ...
    # Get recommended next topics for the user's semester
    cursor.execute('''
        SELECT t.*, s.name as subject_name, s.semester
        FROM topics t
        JOIN subjects s ON t.subject_id = s.id
        LEFT JOIN user_progress up ON t.id = up.topic_id AND up.user_id = ?
        WHERE (up.id IS NULL OR up.completion_status != 'completed') AND s.semester = ?
        ORDER BY t.order_index
        LIMIT 8
    ''', (user_id, current_semester))
    ...
    # Get recommended video resources for the next topics (semester-limited)
    recommended_videos = []
    topic_ids = [t['id'] for t in next_topics]
    if topic_ids:
        placeholders = ','.join('?' for _ in topic_ids)
        query = f"SELECT * FROM learning_resources WHERE type = 'video' AND topic_id IN ({placeholders}) ORDER BY id LIMIT 5"
        cursor.execute(query, topic_ids)
        recommended_videos = [dict(r) for r in cursor.fetchall()]
